...
for i = 1:numleaves
    ...
    if numIntersectionPoints == 0 || numIntersectionPoints == 1
        ...
        % index of Leaf
        connectivity{ielno}(1,3) = idxLeaf;
        % knot vector number
        ...
        %index of Leaf
        connectivity{ielno}(1,3) = idxLeaf;
        connectivity{ielno+1}(1,3) = idxLeaf;
        % knot vector number
...
for iel = 1:numel
    ...
    for n = 1 : nel
        ...
        connectivity{iel}(1,5+n) = nodeNumber;
    end
    % remove repeated values without changing the order
    connectivity{iel}(1,6:end) = unique(connectivity{iel}(1,6:end),'stable');
    % update number of nodes per element
    nel = size(connectivity{iel}(6:end),2);
    
    if sum(coor( connectivity{iel}(1,6:5+nel), 7)) < 0 % outside
        connectivity{iel}(1,4) = 1;
    else % inside
        connectivity{iel}(1,4) = 2;
    end
    
    connectivity{iel}(1,5) = nel;
